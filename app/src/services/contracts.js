...
export default class Contracts {
    ...
    static setNetwork = (networkId) => {
        ...
        Social = web3.eth.contract(socialConfig.abi).at(socialConfig.networks[selectedNetwork].address);
    };

    static Social() {
        if (!Social) throw new Error(`You must first define the network. Call Contract.setNetwork}`)
        return Social;
    }
